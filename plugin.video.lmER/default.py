...
def GetTitles(url, text):
    try:
        ...
        content = html
        match = re.compile('<li class="cat-item cat-item-.+?"><a href="http://latestdude.com/category/(.+?)" >(.+?)</a>').findall(content)
        for url, name in match:
                addon.add_directory({'mode': 'GetTitles1', 'url': 'http://latestdude.com/category/' + url, 'listitem': listitem, 'text': name.strip()}, {'title': name.strip()}, img=IconPath + 'icon.png', fanart=FanartPath + 'fanart.jpg')
...
def GetTitles1(url, text, img):
    try:
        ...
        listitem = GetMediaInfo(html)
        content = html
        match = re.compile('<div class="item">\s*?<a href="http://latestdude.com/(.+?)">\s*?<div class="image">\s*?<img src="(.+?)" alt="(.+?)" />\s*?<span class="player"></span>\s*?<span class="imdb"><b><b class="icon-star"></b></b>(.+?)</span></div>').findall(content)
        match1 = re.compile("href='http://latestdude.com/(.+?)'>.+?&.+?</a><a rel='nofollow' class=.+?'").findall(content)
        for url, img, name, imdb in match:
                addon.add_directory({'mode': 'GetLinks', 'url': 'http://latestdude.com/' + url, 'listitem': listitem, 'text': name.strip(), 'img': img}, {'title': name.strip() + ' : IMDB = ' + imdb}, img= img, fanart=FanartPath + 'fanart.jpg')
        for url in match1:
                addon.add_directory({'mode': 'GetTitles1', 'url': 'http://latestdude.com/' + url, 'listitem': listitem, 'text': url}, {'title': 'Next Page...'}, img= 'http://raumatiroadsurgery.co.nz/img/arrow.png', fanart=FanartPath + 'fanart.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    except:
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def MainMenu(url, text):    #homescreenserie
        ...
        addon.add_directory({'mode': 'GetTitles1', 'section': 'ALL', 'url': BASE_URL + '/category/english/'}, {'title':  '[COLOR blue][B]Latest Added[/B] [/COLOR]>>'}, img=IconPath + 'icon.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles1', 'section': 'ALL', 'url': BASE_URL + '/category/1080p/'}, {'title':  '[COLOR blue][B]1080p movies [/B] [/COLOR]>>'}, img=IconPath + 'icon.png', fanart=FanartPath + 'fanart.jpg')
