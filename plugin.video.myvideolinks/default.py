...
def GetLinks2(url):                                            
        ...
        listitem = GetMediaInfo(html)
        content = html
        match1 = re.compile('<a id="downloadbutton" onclick="return doDownload.+?" href="(.+?)"').findall(content)
        listitem = GetMediaInfo(content)
        for url in match1:
                addon.add_directory({'mode': 'PlayVideo1', 'url': url, 'listitem': listitem}, {'title':  'load stream - ' + url}, img=IconPath + 'watch.png', fanart=FanartPath + 'fanart.png')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
