...
addon_id = 'plugin.video.film-streamingx'
plugin = xbmcaddon.Addon(id=addon_id)
DB = os.path.join(xbmc.translatePath("special://database"), 'film-streamingx.db')
net = Net()
addon = Addon('plugin.video.film-streamingx', sys.argv)
BASE_URL = 'http://film-streamingx.com/'
...
def GetTitles(section, url, startPage= '1', numOfPages= '1'): 
        ...
        for page in range( start, end):
                if ( page != start):
                        ...
                        html = net.http_GET(pageUrl).content                      
                match = re.compile('<div class="item">\s*?<h2>(.+?)</h2>\s*?<div class="peli">\s*?<img src="(.+?)" alt=".+?"/>\s*?<a href="(.+?)"><span class="player">', re.DOTALL).findall(html)
...
def GetLinks(section, url):
        ...
        content = html
        match2 = re.compile('<div id="div1"><iframe src="(.+?)" scrolling="no"').findall(content)
        match3 = re.compile('<p>(.+?)<A HREF=".+?">.+?</A></p>').findall(content)
        listitem = GetMediaInfo(content)
        for name in match3:
                addon.add_directory({'mode': 'GetLinks1', 'url':  url, 'listitem': listitem}, {'title': '[COLOR lawngreen][B]' + name  + '[/B][/COLOR]'}, img= 'http://film-streamingx.com/wp-content/uploads/2015/07/logo22.png', fanart= 'http://imgprix.com/web/wallpapers/private-cinema-room/2560x1600.jpg')
        for url in match2:
                addon.add_directory({'mode': 'GetLinks1', 'url':  url, 'listitem': listitem}, {'title':  '[COLOR blue]Get Direct Link[/COLOR] >>'}, img= 'https://openload.io/assets/img/logo-alpha.png', fanart= 'http://imgprix.com/web/wallpapers/private-cinema-room/2560x1600.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetLinks1(url):                                            
        ...
        listitem = GetMediaInfo(html)
        content = html
        match1 = re.compile('<source type=".+?" src="(.+?)">').findall(content)
        listitem = GetMediaInfo(content)
        for url in match1:
...
def GenreMenu(): 
        ...
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/oscar/',
        ...
        addon.add_directory({'mode': 'GetTitles', 'section': 'ALL', 'url': BASE_URL + '/category/short/',
...
def GetSearchQuery9():
	...
	if (keyboard.isConfirmed()):
                ...
                return
def Search9(query):
        url = 'http://film-streamingx.com/?s=' + query
        url = url.replace(' ', '+')
        print url
        html = net.http_GET(url).content
