...
def GetTitles4(url, text): #year
    try:
        ...
        content = html
        match = re.compile("<a href='http://coolmoviezone.org/tag/(.+?)' class='tag-link.+?' title='.+?topics' style='font-size.+?'>(.+?)</a>").findall(content)
        for url, name in match:
                addon.add_directory({'mode': 'GetTitles3', 'url': 'http://coolmoviezone.org/tag/' + url, 'listitem': listitem, 'text': name.strip()}, {'title': name.strip()}, img=IconPath + 'icon.png', fanart=FanartPath + 'fanart.jpg')
...
def GetTitles2(url, text): #gen
    try:
        ...
        listitem = GetMediaInfo(html)
        content = html
        match = re.compile('<li class="cat-item cat-item-.+?"><a href="http://coolmoviezone.org/category/(.+?)" title=".+?">(.+?)</a>').findall(content)
        for url, name in match:
                addon.add_directory({'mode': 'GetTitles3', 'url': 'http://coolmoviezone.org/category/' + url, 'listitem': listitem, 'text': name.strip()}, {'title': name.strip()}, img=IconPath + 'icon.png', fanart=FanartPath + 'fanart.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
    except:
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def Search(query):
        url = 'http://coolmoviezone.org/index.php?s=' + query
        print url
        html = net.http_GET(url).content
